...
//infix
void buildInfixTree(tnode** root, char infix[], int *pos){
    if(*root == NULL){
        *root = createNewNode(infix[*pos]);
    }
        
    if(isOperator(infix[*pos])){
        *pos = *pos + 1;
        (*root)->left = createNewNode(infix[*pos]);
        buildInfixTree(&(*root)->left, infix, pos);
        
        *pos = *pos + 1;
        (*root)->right = createNewNode(infix[*pos]);
        buildInfixTree(&(*root)->right, infix, pos);
    }
